...
Human の操作（Hard Drop と、それに続く Line Clear・Attack 計算）も同じフレームの処理に含めて測る。
Human の盤面には通常の Gravity がかかる（Lightweight の CPU の盤面だけ Gravity 0）。

### 計測結果（2026-09-26 / Apple Silicon / Godot 4.7.2 / Seed 20260922）

| Player | 平均 (ms) | 最大 (ms) | FPS 換算 | Detailed CPU | Top Out | 決着 |
|---|---|---|---|---|---|---|
| 2 | 0.060 | 0.141 | 1000+ | 0 | 0 | 決着 |
| 10 | 0.136 | 0.635 | 1000+ | 0 | 7 | 時間切れ |
| 30 | 0.476 | 4.520 | 1000+ | 0 | 25 | 時間切れ |
| 50 | 0.883 | 2.902 | 1000+ | 0 | 47 | 時間切れ |
| 99 | 1.957 | 13.998 | 511 | 0 | 89 | 時間切れ |

99 人でも平均 2.0 ms で、1 フレームの予算（16.7 ms）の約 1 割。**平均では 60 FPS に十分届く**。

ただし**最大値が跳ねる**（99 人で 14 ms。2026-09-22 の計測では 30 人で 134 ms も出ていた）。脱落が重なったフレームで
...
99 人戦では 98 体中 89 体が実際に Top Out して脱落し、残りが畳み込みになった
（2026-09-22 は 79 体。#89 で Lightweight の掘る速さを受ける Attack より遅くしたため増えた）。
...
これを 4 回繰り返し、回ごとに計測順を逆にする（先に測る側が起動直後の影響を受けて偏るため）。
各回の順序も出力する。表は 4 回ぶんのサンプルをまとめた値。

### 計測結果（2026-09-26 / Apple Silicon / Godot 4.7.2 / CPU 98 体 / 4 回）

| 分散 | 平均 (ms) | p99 (ms) | 最大 (ms) | 1 フレームで動かす CPU |
|---|---|---|---|---|
| なし | 0.0149 | 0.0840 | 0.1100 | 98 |
| 2 分割 | 0.0209 | 0.0560 | 0.0890 | 49 |
| 4 分割 | 0.0117 | 0.0280 | 0.0510 | 25 |
| 8 分割 | 0.0072 | 0.0150 | 0.0340 | 13 |

分散しても、各 CPU に渡す**ゲーム内時間の合計は変わらない**（時間整合性。#47 の制約）。
ただし更新関数の呼び出しは、経過時間をまとめて渡すぶん減る。処理の総量が同じかどうかは、
この計測では確かめていない。平均は 2 分割で「なし」より大きくなっており、分散すれば平均が
必ず下がるわけではない。

効くのは重いフレームで、p99 が 0.084 ms → 0.015 ms（8 分割）まで下がる。Human Input は
前のフレームの処理が終わるまで待つため、重いフレームが減るほど Input の待ちも短くなる
（MVP 受入条件 29）。

既定は 4 分割（`CpuSchedulePolicy.slice_count`）。この計測は Lightweight CPU だけで、
元々軽いので差は小さい。Detailed CPU（1 手 6〜55 ms。`benchmark_cpu_search.gd` の計測）が
混ざる場合は効果が大きくなると**予測される**が、Detailed を混ぜた分散の効果は測っていない。

### 以前の計測（2026-09-22）について
